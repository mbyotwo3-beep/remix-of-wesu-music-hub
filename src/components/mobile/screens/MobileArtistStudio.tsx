    ...
    <div className="flex flex-col h-full">
      ...
      <Tabs defaultValue="upload" className="flex-1 flex flex-col">
        <TabsList className="mx-4 mb-2 flex h-10 overflow-x-auto shrink-0">
          <TabsTrigger value="upload" className="text-xs shrink-0">Upload</TabsTrigger>
          <TabsTrigger value="albums" className="text-xs shrink-0">Albums</TabsTrigger>
          <TabsTrigger value="collabs" className="text-xs shrink-0">Collabs</TabsTrigger>
          <TabsTrigger value="label" className="text-xs shrink-0">Label</TabsTrigger>
          <TabsTrigger value="features" className="text-xs shrink-0">Features</TabsTrigger>
          <TabsTrigger value="payouts" className="text-xs shrink-0">Payouts</TabsTrigger>
        </TabsList>
        <div className="flex-1 overflow-y-auto">
          ...
          <TabsContent value="collabs" className="mt-0 p-4">
            <p className="text-sm text-muted-foreground">Use the web Artist Studio for full collaborator management.</p>
          </TabsContent>
          <TabsContent value="label" className="mt-0 p-4">
            <p className="text-sm text-muted-foreground">Use the web Artist Studio to manage label invites.</p>
          </TabsContent>
          <TabsContent value="features" className="mt-0 p-4">
            <p className="text-sm text-muted-foreground">Use the web Artist Studio to manage feature availability.</p>
          </TabsContent>
  ...
  return (
    ...
    >
      ...
      <label className="block text-sm">
        Audio file
        <input
          required type="file" accept="audio/*"
...
function AlbumsTab() {
  ...
  const { data: albums } = useQuery({ queryKey: ["my-albums"], queryFn: () => listFn(), retry: false });
  const m = useMutation({ mutationFn: createFn, onSuccess: () => qc.invalidateQueries({ queryKey: ["my-albums"] }) });
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        />
        <input
          type="number" placeholder="Price ZMW"
          className="w-full min-h-[44px] px-3 rounded-lg bg-card border border-border text-sm"
          value={form.price}
          onChange={(e) => setForm({ ...form, price: Number(e.target.value) })}
        />
        <input type="file" accept="image/*" className="text-sm" onChange={(e) => setCover(e.target.files?.[0] ?? null)} />
        <button disabled={busy} className="w-full min-h-[44px] bg-primary text-primary-foreground rounded-xl font-semibold text-sm disabled:opacity-50">
          Create album
        </button>
      </form>
      <div>
        ...
        {(albums ?? []).length === 0
          ...
          : (albums ?? []).map((a: any) => (
              <div key={a.id} className="py-2 text-sm border-b border-border">{a.title}</div>
            ))
        }
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      <div className="bg-card border border-border rounded-xl p-4">
        ...
        <p className="text-2xl font-bold">ZMW {Number(overview?.totalRevenueZmw ?? 0).toFixed(2)}</p>
      </div>
      <form
      ...
      >
        ...
        <input
          required type="number" min="1" step="0.01" placeholder="Amount"
          className="w-full min-h-[44px] px-3 rounded-lg bg-card border border-border text-sm"
          value={form.amount}
        ...
        <input
          required placeholder="Phone / account number"
          className="w-full min-h-[44px] px-3 rounded-lg bg-card border border-border text-sm"
          value={form.destination}
          onChange={(e) => setForm({ ...form, destination: e.target.value })}
        />
        {m.error && <p className="text-xs text-destructive">{(m.error as Error).message}</p>}
        <button disabled={m.isPending} className="w-full min-h-[44px] bg-primary text-primary-foreground rounded-xl font-semibold text-sm disabled:opacity-50">
          Request payout
        </button>
      </form>
    </div>
