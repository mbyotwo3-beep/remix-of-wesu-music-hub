...
function LabelsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12">
      ...
      <p className="text-muted-foreground mb-8">Independent and major labels publishing on Wesu+.</p>
      {labels.length === 0 ? (
        <div className="bg-card border border-border rounded-2xl p-12 text-center">
          <p className="text-muted-foreground">No labels yet. <Link to="/apply-label" className="text-primary underline">Apply to start one</Link>.</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {labels.map((l: any) => (
            <Link key={l.id} to="/labels/$slug" params={{ slug: l.slug }} className="bg-card border border-border rounded-2xl p-5 hover:border-primary transition">
              <div className="size-16 rounded-xl bg-secondary mb-3 overflow-hidden">
                {l.logo_url && <img src={l.logo_url} alt={l.name} className="w-full h-full object-cover" />}
              </div>
