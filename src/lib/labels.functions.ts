...
export const applyForLabel = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: row, error } = await supabase.from("labels").insert({
      name: data.name, slug, owner_user_id: userId,
      bio: data.bio ?? null, contact_email: data.contact_email ?? null, logo_url: data.logo_url ?? null,
...
export const getMyLabel = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data } = await context.supabase.from("labels").select("*").eq("owner_user_id", context.userId).maybeSingle();
    return data;
  });
...
export const getLabelBySlug = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: label } = await sb.from("labels").select("id, name, slug, bio, logo_url, status").eq("slug", data.slug).eq("status", "approved").maybeSingle();
    if (!label) return null;
    const { data: roster } = await sb.from("artists").select("id, name, avatar_url, monthly_listeners").eq("label_id", (label as any).id).eq("status", "approved");
    return { label, roster: roster ?? [] };
  });

export const listApprovedLabels = createServerFn({ method: "GET" })
  .handler(async () => {
    const { createClient } = await import("@supabase/supabase-js");
    const sb = createClient(process.env.SUPABASE_URL!, process.env.SUPABASE_PUBLISHABLE_KEY!, {
      auth: { persistSession: false, autoRefreshToken: false },
    });
    const { data } = await sb.from("labels").select("id, name, slug, bio, logo_url").eq("status", "approved").order("name");
    return data ?? [];
  });
...
export const inviteArtistToLabel = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { error } = await supabase.from("label_artists").insert({
      label_id: data.label_id, artist_id: data.artist_id,
      royalty_pct: data.royalty_pct ?? 80, status: "invited",
    } as any);
    if (error) throw new Error(error.message);
    await audit(userId, "label.invite_artist", "label", data.label_id, { artist_id: data.artist_id });
    return { ok: true };
  });
...
export const respondToLabelInvite = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    if (data.accept) {
      const { data: row, error } = await supabase.from("label_artists")
        .update({ status: "active", joined_at: new Date().toISOString() } as any)
        .eq("id", data.id).select("label_id, artist_id").single();
      if (error) throw new Error(error.message);
      // attach label to artist
      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
      await supabaseAdmin.from("artists").update({ label_id: (row as any).label_id }).eq("id", (row as any).artist_id);
      await audit(userId, "label.invite.accept", "label_artists", data.id);
    } else {
      await supabase.from("label_artists").update({ status: "removed" } as any).eq("id", data.id);
...
export const setArtistRoyalty = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (error) throw new Error(error.message);
    await audit(context.userId, "label.royalty.set", "label_artists", data.id, { royalty_pct: data.royalty_pct });
    return { ok: true };
  });
...
export const removeArtistFromLabel = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (row) {
      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
      await supabaseAdmin.from("artists").update({ label_id: null }).eq("id", (row as any).artist_id);
    }
...
export const listLabelRoster = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    const { data: rows } = await context.supabase
      .from("label_artists")
      .select("id, status, royalty_pct, joined_at, artists!inner(id, name, avatar_url, monthly_listeners)")
...
export const requestLabelPayout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (error) throw new Error(error.message);
    await audit(context.userId, "label.payout.request", "label", data.label_id, { amount: data.amount });
    return { ok: true };
  });
