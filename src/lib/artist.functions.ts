...
async function audit(
  ...
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.warn(
      `[Audit Log Bypass] actor_id: ${actorId}, action: ${action}, target: ${target_type}/${target_id}`,
    );
    return;
  }
...
/** Returns the caller's existing artist row (id + status) or null if they haven't applied yet. */
export const getMyArtistApplication = createServerFn({ method: "GET" })
