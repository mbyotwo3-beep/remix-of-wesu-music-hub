...
export const listFeaturedSlots = createServerFn({ method: "GET" })
  .handler(async () => {
...
export const listAllFeaturedAdmin = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    await ensureStaff(context.supabase, context.userId);
    const { data } = await context.supabase.from("featured_slots").select("*").order("slot_type").order("position");
    return data ?? [];
  });

// Public helper to resolve targets into display payloads for the home page.
export const getHomeFeatured = createServerFn({ method: "GET" })
  .handler(async () => {
    const { createClient } = await import("@supabase/supabase-js");
    const sb = createClient(process.env.SUPABASE_URL!, process.env.SUPABASE_PUBLISHABLE_KEY!, {
      auth: { persistSession: false, autoRefreshToken: false },
    });
    const { data: slots } = await sb.from("featured_slots")
      .select("*").eq("active", true).order("position");
    if (!slots?.length) return [];
    const resolved = await Promise.all(slots.map(async (s: any) => {
      let target: any = null;
      if (s.target_type === "song") {
        const { data } = await sb.from("songs").select("id, title, cover_url, artist_id, price").eq("id", s.target_id).maybeSingle();
        target = data;
      } else if (s.target_type === "album") {
        const { data } = await sb.from("albums").select("id, title, cover_url, artist_id").eq("id", s.target_id).maybeSingle();
        target = data;
      } else if (s.target_type === "artist") {
        const { data } = await sb.from("artists").select("id, name, avatar_url").eq("id", s.target_id).maybeSingle();
        target = data;
      } else if (s.target_type === "label") {
        const { data } = await sb.from("labels").select("id, name, slug, logo_url").eq("id", s.target_id).maybeSingle();
        target = data;
      }
      return { ...s, target };
    }));
